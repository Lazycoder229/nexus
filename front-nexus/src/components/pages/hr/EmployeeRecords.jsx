...
const EmployeeRecords = () => {
  ...
  const fetchEmployees = async () => {
    try {
      const response = await axios.get(`${API_BASE}/api/employees`, { params: filters });
  ...
  const fetchSummary = async () => {
    try {
      const response = await axios.get(`${API_BASE}/api/employees/summary`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.employee_id) {
        await axios.put(`${API_BASE}/api/employees/${formData.employee_id}`, formData);
      } else {
              ...
              {employees.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).length ? (
                employees.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((employee) => (
                  <tr key={employee.employee_id} className="text-sm text-slate-700 dark:text-slate-200 hover:bg-indigo-50/50 dark:hover:bg-slate-700 transition duration-150">
                    ...
                    <td className="px-4 py-2">
                      <span
                        className={`inline-flex items-center px-2 py-0.5 text-xs font-medium rounded-full ${employee.employment_status === "Active"
