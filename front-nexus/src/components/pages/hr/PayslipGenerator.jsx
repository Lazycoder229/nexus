...
const PayslipGenerator = () => {
  ...
  const handleAutoCreatePayslips = async () => {
    ...
    if (
      !window.confirm(
        "Auto-generate payslips for all eligible employees in this period?",
      )
    ) {
      return;
    }
  ...
  return (
    <div className="dark:bg-slate-900 p-3 sm:p-4 transition-colors duration-500">
      <div className="w-full max-w-7xl mx-auto space-y-4 font-sans">
                    ...
                    displayedSetups.map((setup) => (
                      <tr
                        ...
                        className={`text-sm text-slate-700 dark:text-slate-200 hover:bg-indigo-50/50 dark:hover:bg-slate-700 transition duration-150 cursor-pointer ${
                          selectedSetup === setup.payroll_setup_id
                            ? "bg-indigo-50 dark:bg-slate-700/70"
                            : ""
                        }`}
                        onClick={() => {
                      ...
                      >
                        ...
                        >
                          <div className="flex gap-2 justify-end">
                            ...
                            <button
                              onClick={() =>
                                handleDeleteSetup(setup.payroll_setup_id)
                              }
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mt-3 text-sm text-slate-700 dark:text-slate-200">
              <span className="text-xs sm:text-sm">
                ...
                <span className="font-semibold">
                  {Math.min(
                    (setupCurrentPage - 1) * itemsPerPage + 1,
                ...
                of{" "}
                <span className="font-semibold">{filteredSetups.length}</span>{" "}
                periods
              </span>
              <div className="flex gap-1 items-center mt-2 sm:mt-0">
                ...
                <button
                  onClick={() =>
                    setSetupCurrentPage((p) => Math.min(p + 1, setupTotalPages))
                  }
                  disabled={
                    setupCurrentPage === setupTotalPages ||
                    setupTotalPages === 0
                  }
                  className="p-1.5 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-100 dark:hover:bg-slate-600 transition-colors"
                >
              ...
              {selectedSetup && (
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-1.5 rounded-md font-medium transition-colors text-sm border shadow-sm whitespace-nowrap bg-amber-600 text-white hover:bg-amber-700 border-amber-700 dark:border-amber-600 shadow-md shadow-amber-500/30 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <Zap size={14} />
            ...
            {selectedSetup ? (
              <>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center mt-3 text-sm text-slate-700 dark:text-slate-200">
                  <span className="text-xs sm:text-sm">
                    ...
                    <span className="font-semibold">
                      {Math.min(
                        (payslipCurrentPage - 1) * itemsPerPage + 1,
                  ...
                  <div className="flex gap-1 items-center mt-2 sm:mt-0">
                    <button
                      onClick={() =>
                        setPayslipCurrentPage((p) => Math.max(p - 1, 1))
                      }
                    ...
                    <button
                      onClick={() =>
                        setPayslipCurrentPage((p) =>
                          Math.min(p + 1, payslipTotalPages),
                        )
                      }
                      disabled={
                        payslipCurrentPage === payslipTotalPages ||
                        payslipTotalPages === 0
                      }
                      className="p-1.5 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-100 dark:hover:bg-slate-600 transition-colors"
                    >
              ...
              <div className="p-8 text-center border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-800">
                ...
                <p className="text-sm text-slate-400 dark:text-slate-500">
                  You can create a new payroll period from the "Payroll Periods"
                  tab.
                </p>
              </div>
            )}
        ...
        {showPayslipModal && (
          <div
            className="fixed inset-0 bg-black/30 flex items-center justify-center p-2 z-50 transition-opacity duration-300 overflow-y-auto"
            onClick={() => setShowPayslipModal(false)}
          >
            <div
              className="bg-white dark:bg-slate-800 rounded-lg shadow-2xl w-full max-w-4xl my-8 transform transition-transform duration-300 scale-100 border border-slate-200 dark:border-slate-700 max-h-[90vh] overflow-y-auto"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Modal Header */}
