...
import { CheckCircle, XCircle, Clock, Eye, Search, ChevronLeft, ChevronRight, FileText, Plus } from "lucide-react";
...
const GradeEntryApproval = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        alert(editingEntry ? "Grade entry updated successfully" : "Grade entry created successfully");
        handleCloseModal();
        fetchGradeEntries();
      } else {
  ...
  const getApprovalBadge = (status) => {
    const statusColors = {
      ...
      revision_needed: "bg-orange-100 dark:bg-orange-900/30 text-orange-800 dark:text-orange-400",
    };
    ...
    <div className="dark:bg-slate-900 p-3 sm:p-4 transition-colors duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
          ...
          <p className="text-2xl font-bold text-yellow-600 dark:text-yellow-400">48</p>
        </div>
        <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
        ...
        <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
          ...
          <p className="text-2xl font-bold text-indigo-600 dark:text-indigo-400">1,248</p>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700 bg-white dark:bg-slate-800">
              {(() => {
                ...
                const filtered = gradeEntries.filter((entry) => {
                  const matchesSearch =
                    entry.student_first_name?.toLowerCase().includes(searchTerm) ||
                    entry.student_last_name?.toLowerCase().includes(searchTerm) ||
                ...
                return paginatedData.map((entry) => (
                  <tr key={entry.entry_id} className="text-sm text-slate-700 dark:text-slate-200 hover:bg-indigo-50/50 dark:hover:bg-slate-700 transition duration-150">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, raw_score: e.target.value })}
                    className="w-full px-3 py-2 text-sm border border-slate-300 dark:border-slate-600 rounded-md dark:bg-slate-700 dark:text-white focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                    required
                  />
                </div>
