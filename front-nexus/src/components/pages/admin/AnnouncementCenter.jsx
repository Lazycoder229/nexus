...
import { Plus, Edit, Trash2, Search, ChevronLeft, ChevronRight, Bell, Eye } from "lucide-react";
...
const AnnouncementCenter = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/events/announcements?${params}`);
  ...
  const fetchStatistics = async () => {
    try {
      const response = await axios.get("http://localhost:5000/api/events/announcements/statistics");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (current) {
        await axios.put(`http://localhost:5000/api/events/announcements/${current.announcement_id}`, formData);
      } else {
        await axios.post("http://localhost:5000/api/events/announcements", formData);
      }
      fetchData();
      fetchStatistics();
      closeModal();
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Delete this announcement?")) {
      try {
        await axios.delete(`http://localhost:5000/api/events/announcements/${id}`);
        fetchData();
        fetchStatistics();
      } catch (error) {
        console.error("Error:", error);
  ...
  const closeModal = () => {
    setShowModal(false);
    setCurrent(null);
    setFormData({ title: "", content: "", announcement_type: "General", priority: "Normal", target_audience: "All", target_year_levels: "", target_courses: "", status: "Draft", publish_date: new Date().toISOString().split('T')[0], expiry_date: "", is_pinned: false });
  };
      ...
      <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-700 pb-3">
        ...
        {statistics && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <div className="bg-white dark:bg-slate-800 rounded-md border border-slate-200 dark:border-slate-700 shadow-sm p-4"><p className="text-xs font-medium text-slate-600 dark:text-slate-400">Total</p><p className="text-2xl font-bold text-indigo-600 dark:text-indigo-400 mt-1">{statistics.total_announcements}</p></div>
            <div className="bg-white dark:bg-slate-800 rounded-md border border-slate-200 dark:border-slate-700 shadow-sm p-4"><p className="text-xs font-medium text-slate-600 dark:text-slate-400">Published</p><p className="text-2xl font-bold text-green-600 dark:text-green-400 mt-1">{statistics.published_count}</p></div>
            <div className="bg-white dark:bg-slate-800 rounded-md border border-slate-200 dark:border-slate-700 shadow-sm p-4"><p className="text-xs font-medium text-slate-600 dark:text-slate-400">Draft</p><p className="text-2xl font-bold text-gray-600 dark:text-gray-400 mt-1">{statistics.draft_count}</p></div>
            <div className="bg-white dark:bg-slate-800 rounded-md border border-slate-200 dark:border-slate-700 shadow-sm p-4"><p className="text-xs font-medium text-slate-600 dark:text-slate-400">Views</p><p className="text-2xl font-bold text-purple-600 dark:text-purple-400 mt-1">{statistics.total_views}</p></div>
          </div>
        )}
        ...
        <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-700 bg-white dark:bg-slate-800">
            {currentItems.length > 0 ? (
              ...
              <tr key={item.announcement_id} className="text-sm text-slate-700 dark:text-slate-200 hover:bg-indigo-50/50 dark:hover:bg-slate-700 transition duration-150">
                <td className="px-4 py-2">
                  <div className="font-semibold">{item.is_pinned && '📌 '}{item.title}</div>
                ...
                <td className="px-4 py-2">
                  {item.publish_date ? new Date(item.publish_date).toLocaleDateString() : 'N/A'}
                </td>
                <td className="px-4 py-2">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-2xl w-full max-w-2xl transform transition-transform duration-300 scale-100 border border-slate-200 dark:border-slate-700 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="p-4 space-y-3">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="is_pinned" className="text-xs font-medium text-slate-700 dark:text-slate-300">
                  Pin to Top
                </label>
              </div>
