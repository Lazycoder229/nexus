...
import { Settings, Plus, Edit, Trash2, AlertCircle, Search, ChevronLeft, ChevronRight, FileText } from "lucide-react";
...
const GradeComputationSetup = () => {
  ...
  const getComponentTypeBadge = (type) => {
    const typeColors = {
      ...
      assignment: "bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-400",
      project: "bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-400",
      attendance: "bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400",
      participation: "bg-indigo-100 dark:bg-indigo-900/30 text-indigo-800 dark:text-indigo-400",
    ...
    <div className="dark:bg-slate-900 p-3 sm:p-4 transition-colors duration-500">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-sm text-slate-600 dark:text-slate-400">Total Weight</h3>
          ...
          <span className="text-xs sm:text-sm">
            ...
            <span className="font-semibold">{(() => {
              ...
              const filtered = settings.filter((setting) => {
                const matchesSearch =
                  setting.component_name?.toLowerCase().includes(searchTerm) ||
                  setting.component_type?.toLowerCase().includes(searchTerm) ||
